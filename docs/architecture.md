...
- Root extension: `extensions.VRMXT_sprite_particle` (legacy `VRMXT_vfx` / `VRMXT_particle` not read)
...
- `VrmxtMaterialsOverrideMaterialize` (Editor) — create/update durable `.mat` assets from override JSON, assign `OverrideMaterial`, swap that asset onto matching renderer slots; Swap Back to MToon restores `SourceMaterial` on renderers.
